...
class UpdateAgent implements ICheckAgent, IUpdateAgent, IDownloadAgent {
    ...
    public void setInstall( IInstall install){
        mInstall = install;
    }
    ...
    void doInstall() {
        try {
            String md5 = mContext.getSharedPreferences(PREFS, 0).getString(KEY_UPDATE, "");
            File apk = new File(mContext.getExternalCacheDir(), md5 + ".apk");
            if (UpdateUtil.verify(apk, md5)) {
                mInstall. install(mContext, apk, mInfo.isForce);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
