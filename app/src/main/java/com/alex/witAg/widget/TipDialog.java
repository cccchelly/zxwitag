...
public class TipDialog {
     ...
     * @return
     */
    public TipDialog(Activity con) {
        this.activity = con;
        init();
    }

    public TipDialog(Activity con, int type) {
        this.activity = con;
        dialogType = type;
        init();
    }

    public TipDialog(Activity con, boolean canceledOntouchOutside) {
        this.activity = con;
    ...
    private void init() {
        dialog = new Dialog(activity, R.style.translucentDialogStyle);
        ...
        if (dialogType == TYPE_SHOW) {
            ...
            lineView.setVisibility(View.GONE);
            selectLin.setVisibility(View.GONE);
            tagIv.setImageResource(R.drawable.pb_login);
    ...
    public void show() {
        ...
        if(activity == null || dialog.isShowing() || activity.isFinishing())return;
        try {
            dialog.show();
        } catch (Exception e) {
            e.printStackTrace();
